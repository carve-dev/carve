...
- **Two trigger contexts.** The recovery agent is invoked from:
  1. **`carve run` failures** — the original M2 use case. Pipeline ran, hit an error, recovery agent diagnoses + patches + retries. Side effects in dev only (the dev target).
  2. **`carve deploy` Phase 1 pre-flight failures** — when M2-14's deploy orchestrator detects a fixable issue during local pre-flight (e.g., stale build vs. drifted prod schema, missing destination column the build expects). The recovery agent handles these because Phase 1 has no prod side effects yet — it's read-only validation. Typical Phase 1 fixes: refine the plan to match drifted schema (`request_replan`), regenerate Snowflake DDL (delegate to snowflake specialist), update the manifest. After a successful Phase 1 fix, M2-14 retries the pre-flight; on success it proceeds to Phase 2 (PR + workflow generation).
- **No auto-fix in CI.** Phases 3-5 of `carve deploy` run in GitHub Actions post-merge against the prod target. Failures there surface as failed GitHub Actions checks; the user investigates and re-runs the workflow manually. The recovery agent is **not** invoked from CI in M2 (deferred to M3 — needs CI auth, branch-write privilege, and a way to round-trip fixes via follow-up PRs). Phase 4's idempotent migrations make manual retry safe.
...
- **Auto-fix in CI (post-merge deploy phases 3-5).** Phase 1 of `carve deploy` (local, read-only) gets recovery-agent help — see the "Two trigger contexts" item above. Phases 3-5 run in GitHub Actions against prod and are NOT auto-fixed by the recovery agent in M2; failures surface as failed CI checks. Auto-fix in CI requires CI auth + branch-write + follow-up-PR mechanics — deferred to M3. The asymmetry is intentional: Phase 1 has no side effects, so AI experimentation is safe; CI runs against prod, where mistakes are expensive and human review is the right gate.
