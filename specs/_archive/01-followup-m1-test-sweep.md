# state-store — M1 test fixture sweep + missing state-store unit tests

> Closes the deferred work from [`state-store`](../capabilities/state-store.md). The Postgres-from-day-one shift broke ~200 M1-era tests that built their own SQLite-backed `Config` or invoked `carve init` via `CliRunner` without a reachable Postgres. This spec sweeps them all and adds three unit tests that state-store ## Tests required but didn't ship.

## Status

- **Status:** Landed (2026-05-20, commit `4429000`)
- **Depends on:** [state-store](../capabilities/state-store.md) — uses the Postgres testcontainers fixture that state-store added to `tests/conftest.py`
- **Blocks:** nothing structurally — but it gates the v0.1 regression bar ("the full M1 test suite passes against Postgres" per state-store ## Acceptance). packaging onward can be developed in parallel; they just need this spec landed before v0.1.0 ships.
...
Bring the M1 test suite back to green against the state-store Postgres state store, in a single sweep. After this spec lands:

- `uv run pytest tests/ -q` is green
- The three state-store ## Tests bullets that have no coverage are covered
- `DEFAULT_STATE_STORE_URL = "postgresql+psycopg://carve:carve@localhost:5432/carve"` is documented as dev-only (security-reviewer Low finding from state-store)
...
- Any new feature, refactor, or runtime behavior change. state-store already landed the runtime correctness; this spec is the test sweep that comes after.
...
- Performance tuning of the testcontainers Postgres fixture. state-store already used a session-scoped container with per-test database — that pattern is fine as-is.
...
- **`tests/core/state/test_database.py`** — exercises `create_engine_from_config`, `initialize_database`, and connection-lifecycle assertions. Rewrite against Postgres. Bullets 1 and 2 of state-store ## Tests land here as new test functions:
  - `test_model_metadata_reflects_against_postgres` — open a Postgres engine, `inspect(engine).get_table_names()` returns the expected M1-shape tables (plus whatever's already added in state-store)
...
> **Updated during implementation (2026-05-20):** The interim "monkeypatch shim required" wording below described the state of the world *before* [`01-followup-database-url-env-precedence` (state-store)](./01-followup-database-url-env-precedence.md) landed. state-store teaches `resolve_state_store_url` to honor `DATABASE_URL` natively, so the monkeypatch in `cli_env` has been removed and the fixture now returns just `{"DATABASE_URL": postgres_state_store_url}`. The instructions below are preserved for historical context; future readers should treat the C1 recipe as "use the `cli_env` fixture and pass `env=cli_env` to `CliRunner.invoke(...)`" — no shim involved.
...
> **Updated during implementation (2026-05-20):** state-store's native `DATABASE_URL` precedence collapsed this fixture to a one-line env dict. The shim signature below is preserved as the as-shipped-then-removed form; the live fixture is now:
>
> ```python
> @pytest.fixture
...
This is the one Bucket C file that needs more than env-override. `carve init`'s post-state-store behavior added new scaffolded files (state_store config in runtime.toml, the Postgres `DATABASE_URL` placeholder in `.env.example`, etc.). The existing assertions about init's output need to be reviewed:
...
3. **Alembic upgrade head on empty Postgres** (bullet 4 from state-store ## Tests): in `tests/migrations/test_migrations.py`, a `test_alembic_upgrade_head_on_empty_postgres` that creates a fresh database, runs `alembic upgrade head`, and asserts the resulting schema's table list and selected column types match expected. This is the schema-shape regression test.
...
- The three state-store ## Tests bullets that had no coverage now have dedicated tests
...
- No production code changes outside the docstring updates (this is a test sweep)
  > **Caveat 1 (test-side shim, applied during implementation 2026-05-20 — RESOLVED 2026-05-20 by state-store):** During this sweep, `cli_env` (`tests/conftest.py`) carried a function-scoped monkeypatch that masked a production gap — `resolve_state_store_url` did not honor `DATABASE_URL` natively during `carve init`. [`01-followup-database-url-env-precedence` (state-store)](./01-followup-database-url-env-precedence.md) landed the native env precedence on the same day, and the `cli_env` fixture has since been collapsed to a one-line env dict with the monkeypatch removed. This caveat is preserved as a record of the interim state; the suite's current behavior matches the spec body as written, with no shim in `cli_env`.
...
- **Why is `test_init_centralized.py` called out separately?** Its assertions test the *shape* of what init writes, not just that init succeeds. state-store changed that shape. So it needs review beyond the mechanical env override.
- **Why ship the dev-only docstring callout in this spec rather than state-store?** Because state-store was already big enough and the credential default landed alongside the bundled-Postgres path (which is packaging). Catching it in this followup keeps state-store focused on what it did and clusters the credential-default discussion with the test sweep where it's small.
...
- **Does C1's env-override actually work today against unmodified production?** *Resolved during implementation (2026-05-20).* No — it requires the `cli_env` fixture to additionally monkeypatch `resolve_state_store_url` so the resolved URL honors `DATABASE_URL` when the bootstrap Config has no override. The production gap (env-var precedence in `resolve_state_store_url`) is tracked in [`01-followup-database-url-env-precedence`](./01-followup-database-url-env-precedence.md). When that lands, the `cli_env` fixture collapses to the one-line dict version originally shown in §Bucket C.
  > **Updated 2026-05-20 (state-store landed):** Native `DATABASE_URL` precedence is now in production (`resolve_state_store_url` reads `os.environ["DATABASE_URL"]` between the explicit-config and legacy-alias steps). The `cli_env` fixture has been collapsed to the one-line `return {"DATABASE_URL": postgres_state_store_url}` form; the monkeypatch is gone. C1 now works against unmodified production.
...
- **Should we also exercise `state_store.url` env-var interpolation in `test_schema.py`?** *Implementation default.* Yes if it doesn't already; state-store's `state_store.py` does the interpolation and state-store's tests probably already cover it but verify during implementation.
