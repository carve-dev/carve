...
- **Static-UI lineage view** — already deferred (spec 11); nothing to render since there's no graph. The hosted cloud UI may build its own visualization later.
- **Embedding / semantic search** (ARCHITECTURE §6.1 layer 5) — see [semantic-search](./semantic-search.md).
...
- **The tradeoff, honestly.** A persisted graph could answer "impact across a 5,000-model project" in one indexed query; investigation pulls the relevant slice per question instead. For the initial audience and the explorer's bounded-context model, investigating the slice is the right call — and it never goes stale. If a future scale need emerges, dbt's manifest is already a complete graph to load on demand; we still wouldn't need a *Carve* store.
