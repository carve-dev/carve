...
> **How Carve authenticates to its model provider** — the credential subsystem the [harness](./harness.md) needs to run at all. Two paths: an **`ANTHROPIC_API_KEY`** env var, or a **Claude-subscription OAuth** flow via `carve auth login` (browser-based, token stored locally + auto-refreshed). Owns the credential **precedence**, the **OAuth flow + token storage/refresh**, `models.toml`'s `auth_mode` + model tiers, and the **OSS-vs-hosted split**. Distinct from API-token auth for REST/MCP (that's [rest-api](./rest-api.md)) and from warehouse/source credentials (that's [connect](./connect.md)/[sql](./sql.md)). *Build status:* shipped. The `ANTHROPIC_API_KEY` path shipped in M1.1; the **single precedence resolver, the SDK-native OAuth bearer path, and `models.toml`'s `auth_mode` + model tiers** shipped in [DELIVERY](../DELIVERY.md) Increment 1b. Carve owns **no** browser OAuth flow or token store — it leans on the `anthropic` SDK + `claude setup-token` (an earlier draft specified a Carve-owned flow; that was corrected after the M1.1-02 investigation).

## Status

- **Status:** Landed (the precedence resolver, SDK-native OAuth bearer, and `auth_mode`/`tiers` schema shipped in DELIVERY Increment 1b; the `ANTHROPIC_API_KEY` path shipped in M1.1)
...
- **Why lean on the SDK, not a Carve-owned OAuth flow.** The `anthropic` SDK already accepts a subscription OAuth bearer (`auth_token=` + the `oauth-2025-04-20` beta header), and `claude setup-token` / `ant auth login` already own the browser exchange, storage, and refresh. A Carve-owned browser flow + `.carve/anthropic_oauth.json` + refresh loop would re-implement that with extra security surface for no gain — so Carve resolves and wires the credential and delegates acquisition. (This corrected an earlier design that specified a Carve-owned flow; the investigation that resolved it is in the M1.1-02 follow-up + [DELIVERY](../DELIVERY.md) Increment 1b.)
...
- **Sequencing.** The `ANTHROPIC_API_KEY` path shipped in M1.1; the precedence resolver (collapsing the four `anthropic.Anthropic(api_key=…)` sites into one `client_factory`), the SDK-native OAuth bearer path, and the `auth_mode`/`tiers` schema land in [DELIVERY](../DELIVERY.md) Increment 1b. The M1.1-02 investigation is resolved: the `anthropic` SDK authenticates a subscription OAuth bearer natively (`auth_token=` + the `oauth-2025-04-20` header) with no adapter, so Carve needs no browser flow, token store, or refresh. Further model-provider work (e.g. Bedrock/Vertex via `ANTHROPIC_BASE_URL`) is a later increment.
