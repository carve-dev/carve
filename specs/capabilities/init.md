...
For users who started with the pre-positioning `targets/<target>/el/<artifact>/` layout (a small set of M1.1 / pre-Pillar-1.1 users), this flag triggers the migration described in [spec 03's *Migration from M1.1*](./layout.md) section. Init creates a git commit "Pre-Carve-layout-migration" first so the user can revert. The migration is one-shot — re-running is a no-op.
...
- **First-run telemetry opt-in prompt.** *Strategy-required.* Should `carve init` prompt the user to opt into anonymous usage telemetry? If yes, what data, opt-in default? This is a community-trust decision worth thinking about deliberately, but it doesn't block the capability specs — defer to a follow-up.
