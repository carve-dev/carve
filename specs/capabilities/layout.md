...
> **Updated during implementation (2026-06-18):** `sync_workspace` gained three keyword-only params over the positional core. `ref: str | None` carries the optional pin (so a `separate-remote` component checks out an exact SHA/tag instead of tracking `branch`), `branch` became `str | None`, `sync_mode="hard"` selects hard-reset vs `git pull`, and `timeout` bounds each git subprocess. The derived workspace dir name is computed by `workspace_dirname(url, ref, branch)` (in `component_locator.py`, imported here so resolution and caching agree) — the `slugify(url) + "-" + ref-or-branch` shape described under *Path resolution* is unchanged.
...
> **Updated during implementation (2026-06-18):** the provenance reader shipped flattened to `src/carve/integrations/provenance.py` (import `carve.integrations.provenance`), not under an `integrations/dlt/` subpackage. Path reference corrected; the convention is unchanged.

- The header is parsed by `src/carve/integrations/provenance.py` (`parse_provenance_header` / `is_carve_generated`) and made available as structured metadata to other parts of the system (e.g., spec 19's `dlt_schema` reader uses it for the component's `destination` binding during lineage investigation; and any future provenance view).
