...
- **Namespace:** callable tools (base tools + `@skill` functions + `mcp:<server>:<tool>`) share the one namespace the loop guards (`loop.py` raises on collision). MCP names are namespaced (`mcp:`) so they can't collide; SkillPacks are content (not in the tool namespace); a user agent granting a name that's both a base tool and a pack resolves to the base tool (logged).
  ...
  > **Grant → executor binding (landed 2026-06-23, Increment 3):** a declarative `tools:` grant is a list of *names*; the registry yields name-only stubs whose executor raises. `core/agents/tool_binding.bind_grant_tools(declared_tools, BindingContext)` is the seam that binds each granted name to its real executor when `SubagentRunner` composes a delegated agent's toolset — the harness base tools from `(project_dir, child-clamped gate, approver)`, caller-injected tools (e.g. a connection-backed `sql`) via `extra_tools` under a bound-name == grant-name precondition, and a still-raising stub for any unbound name (fail-loud). The gate is built from the grant names and still attenuates every call at `child_mode`. This is what makes a declarative agent actually *run* its tools.
