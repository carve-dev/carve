...
```
src/carve/api/__init__.py
src/carve/api/main.py                                   # NEW — FastAPI app factory, middleware wiring, app lifecycle
src/carve/api/auth.py                                   # NEW — bearer token validation, Identity dataclass
src/carve/api/errors.py                                 # NEW — problem+json conversion + exception handlers
src/carve/api/pagination.py                             # NEW — cursor pagination helpers
src/carve/api/idempotency.py                            # NEW — Idempotency-Key middleware + response caching
src/carve/api/streams.py                                # NEW — WebSocket + SSE handlers
src/carve/api/webhooks.py                               # NEW — webhook publisher (consumes events table)
src/carve/api/openapi_meta.py                           # NEW — OpenAPI customization (tags, examples, descriptions)
src/carve/api/health.py                                 # NEW — /healthz, /readyz

src/carve/api/routers/__init__.py
src/carve/api/routers/plans.py                          # NEW — wraps the plan store (from M1.1; HISTORICAL)
src/carve/api/routers/builds.py                         # NEW — wraps Build (from M1.1; HISTORICAL)
src/carve/api/routers/runs.py                           # NEW — wraps runs + jobs from spec 07; supports streams
src/carve/api/routers/deploys.py                        # NEW — git-based deploy from M1.1; supports PR + dry-run
src/carve/api/routers/schedules.py                      # NEW — wraps schedules (from spec 07)
src/carve/api/routers/pipelines.py                      # NEW — wraps pipelines (from spec 08)
src/carve/api/routers/agents.py                         # NEW — wraps agents (from spec 04 + built-ins)
src/carve/api/routers/skills.py                         # NEW — wraps skill registry
src/carve/api/routers/mcp_servers.py                    # NEW — wraps external MCP server registration
src/carve/api/routers/memory.py                         # MODIFY — moves from spec 06 (which now ships only services, not the router)
src/carve/api/routers/metrics.py                        # NEW — runs/costs/agents rollups
src/carve/api/routers/workers.py                        # NEW — read-only list/health of workers
src/carve/api/routers/webhooks.py                       # NEW — webhook subscription CRUD

src/carve/core/state/repositories/tokens.py             # NEW — token CRUD (used by auth middleware)
src/carve/core/state/repositories/idempotency.py        # NEW — Idempotency-Key cache
src/carve/core/state/repositories/webhook_deliveries.py # NEW — delivery attempts + retries

migrations/versions/0010_api_tables.py                  # NEW — tokens, idempotency_keys, webhooks, webhook_deliveries

src/carve/cli/serve.py                                  # MODIFY — wires the API app into the asyncio loop alongside scheduler/workers (per spec 07)

tests/unit/test_api_auth.py
tests/unit/test_api_errors.py
tests/unit/test_api_pagination.py
tests/unit/test_api_idempotency.py
tests/unit/test_api_openapi_schema.py                   # verifies generated OpenAPI matches expected structure
tests/integration/test_api_lifecycle.py                  # carve serve → curl an endpoint → see response
tests/integration/test_api_streams.py                    # WebSocket + SSE consume run events
tests/integration/test_api_webhooks_delivery.py          # webhook fires; subscriber receives signed payload
tests/integration/test_api_routers_full_coverage.py      # every CLI command has a working REST equivalent

docs/api-reference.md                                   # NEW — overview of /api/v1; points to Swagger for full detail
docs/api-authentication.md                              # NEW — how tokens work, how to mint, how to rotate
docs/api-webhooks.md                                    # NEW — subscriber model, payload schemas, signing verification
docs/api-streaming.md                                   # NEW — WebSocket vs SSE; reconnection semantics
```

A note on the spec-06 router file: spec 06 listed `src/carve/api/routers/memory.py` in its files-produced section. Under the consolidated model in this spec, the router is shipped *here* (spec 09) and spec 06 ships only the underlying services (`MemoryLoader`, `MemorySelector`, `MemoryWriter`). When the v0.1 specs land in `/build-spec` order, spec 06 will produce the services first; spec 09 will produce the routers that wrap them. The README in spec 06 should be revised to drop the router file from its file list when spec 09 lands — flagged as a minor coordination point.
