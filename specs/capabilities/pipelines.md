...
- The verdict comes from the on-disk **load package** (`read_latest_load_package`): a clean exit **plus** a `loaded` package with no `failed_jobs`. A clean exit that wrote no new package — or only a **stale** package from a prior run (`DLT_DATA_DIR` is persistent) — is `failed`, never false-green. This explicitly includes the **first-run import-and-exit** case: a component whose entrypoint only *defines* sources runs no load, so the `DLT_DATA_DIR/pipelines` dir is never created — that clean exit is `failed` ("ran no load"), **not** succeeded-with-empty. The component must run a load on exec, per [dlt-engineer](./dlt-engineer.md)'s runnable-entrypoint contract.
