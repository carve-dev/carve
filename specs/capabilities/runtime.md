...
>
> The bullets below remain the spec's full-runtime test target (the **scheduler / schedule-mutation / pause-origin**, the **heartbeat / reaper**, and now the **archiver** bullets are **SHIPPED** by the tests above; the **serve-supervisor / worker-pool crash-recovery** bullets stay **DEFERRED** with their slices):
