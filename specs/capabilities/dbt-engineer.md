...
> **Updated during implementation (2026-06-26):** this unit landed, **completing the dbt-engineer capability**. Recording the **shipped mechanism** so the design body names it (the intent — infer-then-author-in-style + greenfield-scaffold — is unchanged):
> - **Inference is an on-demand callable Tool, not eager auto-inference.** `dbt_conventions` (`integrations/dbt/conventions.py`, `make_dbt_conventions_tool`) infers from the compiled `target/manifest.json` **unioned with** the raw `models/**/*.sql` tree + `dbt_project.yml` — so it works pre-compile — producing a typed `InferredConventions` (per-layer prefixes `stg_`/`int_`/`mart_`/`fct_`/`dim_`, folder layout, per-layer materialization defaults, generic-test coverage). It is **observational**: a partial project infers a partial record, never a fabricated full house. `op=infer` returns the structured record, `op=summary` the markdown. The tool is granted to both the **dbt-engineer** and the **dbt-qa** reviewer, and is the live read behind the `conventions.md` in memory. (Mirrors the dlt brownfield parallel — on-demand, not warehouse-coupled.)
> - **Persistence is the human-invoked `carve memory refresh` CLI**, not the tool: it resolves the same-repo dbt project (root or one level down, via the shared locator), runs `infer_conventions`, and writes `carve/conventions.md` through `MemoryWriter.write_conventions` (atomic; **refuses an empty body** so the agent is never falsely told "no conventions inferred"). No-project → friendly no-op; ambiguous multi-project → exit 2. The dlt side is tool-only; **dbt persists** because this capability's contract is "infers … *into* memory."
> - `check_naming_violation` is the deterministic substrate behind dbt-qa's "a model violating the inferred naming convention is flagged" — a layer with no inferred prefix never produces a violation (only conventions the project actually established are enforced).
> - The eager orchestrator context-assembly memory hook (attaching `conventions.md` to the engineer's bundle automatically) stays **deferred** — it lands with the live goal-classification wiring (plan-build). Today the engineer reads conventions on demand via `dbt_conventions`.

- **Brownfield:** infers the existing project's conventions (naming, layout, materialization, test patterns) into [memory](./memory.md) — on demand via the `dbt_conventions` tool, persisted by `carve memory refresh` — and authors *in that style*, never rewriting existing models unasked (same provenance discipline as dlt).
- **Greenfield:** scaffolds a dbt project (`dbt_project.yml`, a staging/marts layout, sample models + tests) when the user has no dbt yet — shipped as `carve init --with-dbt`. The init-scaffolded `conventions.md` stays **comment-only** (it adds nothing to context until inference runs); `carve memory refresh` / inference is what fills it with real inferred content.

> **The `conventions.md` format (chosen during implementation, 2026-06-26):** a **minimal labelled-markdown** body — a provenance header line marking it Carve-inferred, then a `## Layers` section (per-layer folder / prefix / materialization) and a `## Tests` section (generic tests in use, key columns, source-freshness) — **not** an enforced schema. The [`MemoryLoader`](./memory.md) round-trips it **verbatim** (it reads the file as opaque text), mirroring the freeform `decisions.md`/`standards.md` bodies. User rules in `standards.md` take precedence where they conflict. This is the de-facto format; it is not re-litigated here.
...
- **Unit (brownfield style):** authored SQL matches the inferred `stg_`/`mart_` conventions from a fixture project.
  > **Updated during implementation (2026-06-26):** the inference substrate this rests on is unit-tested in `tests/integrations/dbt/test_conventions.py` over the `tests/integrations/dbt/fixtures/brownfield_project/` fixture — `infer_conventions` detects the `stg_`/`mart_`/`dim_` naming, the staging/marts layout, per-layer materialization, and generic-test patterns; the partial-conventions robustness case (a project that uses only `stg_` infers exactly that, never a fabricated full house); `render_conventions_md` round-trips to loader-readable markdown; and `check_naming_violation` flags a model that departs from the inferred prefix (the substrate behind the dbt-qa "naming convention" flag above).
