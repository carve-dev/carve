...
Formalize the plan/deploy lifecycle. Plans become first-class persisted artifacts with task graphs, cost estimates, file diffs, config-hash validation, and refinement. `carve build` becomes a polite shorthand for plan + interactive confirm + deploy.
...
## Deploy with hash validation

`carve deploy <pipeline_name>`:

```python
def deploy_command(plan_id: str):
    config = load_config()
    ...
    # 7. Update plan as deployed
    plan_store.mark_plan_deployed(plan_id, run_id)
...
class TaskGraphExecutor:
    ...
    def execute(self, plan: Plan) -> str:
        run_id = self.repo.create_run(kind="deploy", target_id=plan.id)
        try:
...
Deploy with: carve deploy <pipeline_name>
```
...
- `carve deploy <pipeline_name>` — execute saved plan
- `carve build "<goal>"` — plan + interactive confirm + deploy
- `carve build "<goal>" --yes` — plan + deploy, no confirm
...
- Hash mismatch on deploy raises `ConfigDriftError`
- Expired plan on deploy raises `PlanExpiredError`
- Refinement creates a child plan with correct `parent_plan_id`
- Guardrail violation blocks deploy
- Deploy records a run with the plan's task graph

## Acceptance criteria

- A plan generated by the orchestrator can be saved, loaded, deployed
- The CLI flow `carve plan → carve deploy` works end-to-end
