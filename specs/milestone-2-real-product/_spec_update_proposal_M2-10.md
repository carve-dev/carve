...
- `GET /api/v1/pipelines/{name}` — pipeline detail (lineage + recent runs)
- `POST /api/v1/pipelines/{name}/runs` — trigger a dev run (`carve run <name>`)
- `POST /api/v1/pipelines/{name}/deploy` — open a prod-deploy PR for this pipeline (`carve deploy <name>`). M2-14 implements the PR mechanics; this endpoint is defined here so the URL surface is stable.
...
- `GET /api/v1/plans/{plan_id}` — plan detail (full design JSON, phase, lineage)
...
- `POST /api/v1/plans/{plan_id}/build` — invoke `carve build`. Materializes `pipelines/<name>/`, upserts the `Pipeline` row, marks the plan `phase="built"`. Returns 202 with a job id (build runs the build agent; takes 30–60s).
...
Match the M1.1-06 state store models, exposing only what's safe:
...
class PlanResponse(BaseModel):
    ...
    goal: str
    design: dict  # the submit_plan design document
    created_at: datetime
    built_at: datetime | None
    built_run_id: str | None
