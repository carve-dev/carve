...
6. Leaves goal classification, two-layer agent selection, impact analysis, pre-scoping, investigation handling, and manual override unchanged.
7. Drops cost/duration/Snowflake-credit estimates from this spec — M2-01's review removed estimates from the Plan model entirely (deferred to M3+).
...
5. **Skipped-agent transparency** — record why each unselected agent was excluded
6. **Delegation to `m1_plan_agent`** — for single-pipeline ingest goals, produce a one-task plan that invokes the existing M1.1 plan agent rather than duplicating its work
...
- extract_load: author Python extract-and-load scripts (M2-03; the build-time
...
`expected_outputs` lists predicted file changes (which files will be created/modified). They're predictions — the actual diffs may differ slightly. M2-01's build coordinator verifies each specialist's actual output against this list. M3 can refine with stricter contract enforcement.
...
- Plan includes invoked agents, skipped agents with reasons, and the task graph
