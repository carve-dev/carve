...
2. **Capability specs — durable; the lowest level of *design* detail.** One per capability *area* (runtime, harness, deploy, lineage, SQL…), organized by **domain, not version**. Each describes the whole capability — behavior, contracts, data model, interfaces, decisions — with **phasing as annotations** ("concurrent subagent fan-out arrives in a later increment"), never as separate per-version files. There is exactly one place to read "how deploy works," and you edit it in place as deploy evolves.
