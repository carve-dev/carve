...
The `append_decision` helper takes `date`, `title`, `body`, `reviewers` and writes this format with consistent spacing. The body is rendered as-is (markdown supported); the structure (heading, Decision/Rationale/Reviewers/Impact bold labels) is enforced by the helper.
