# v0.1-07 — Runtime: scheduler, job queue, workers, heartbeats, reaper, archiver
...
```
src/carve/runtime/__init__.py
src/carve/runtime/scheduler.py                          # NEW — the cron-evaluator loop
src/carve/runtime/job_queue.py                          # NEW — claim, enqueue, dedup helpers
src/carve/runtime/worker.py                             # NEW — worker process loop
src/carve/runtime/worker_pool.py                        # NEW — in-process pool for `carve serve --workers N`
src/carve/runtime/heartbeat.py                          # NEW — per-claim heartbeat task
src/carve/runtime/reaper.py                             # NEW — stale-heartbeat reclaim loop
src/carve/runtime/archiver.py                           # NEW — periodic archive flow for jobs/runs/logs/steps
src/carve/runtime/step_executor_base.py                 # NEW — abstract StepExecutor + StepExecutorRegistry
src/carve/runtime/events.py                             # NEW — runtime event emission (job.*, run.*, step.*, schedule.*)
...
tests/unit/test_optimistic_claim.py                     # NEW
tests/unit/test_heartbeat_loop.py                       # NEW
tests/unit/test_reaper_stale_detection.py               # NEW
tests/unit/test_archiver_verify_then_delete.py          # NEW
tests/integration/test_serve_lifecycle.py               # NEW — `carve serve` startup, graceful shutdown
tests/integration/test_worker_crash_recovery.py         # NEW — kill -9 a worker mid-run; reaper recovers
