...
```
src/carve/cli/init.py                                   # NEW or REWRITE — top-level `carve init` Typer command
src/carve/init/orchestrator.py                          # NEW — drives the init flow
src/carve/init/detect.py                                # NEW — brownfield detection for dbt + dlt + git + docker
...
src/carve/init/templates/                               # NEW — Jinja2 templates rendered by init
    ...
    decisions.md.j2                                     #   templated comment-only "what goes here"
    .env.example.j2                                     #   env-var template with placeholders + comments
    .gitignore.j2                                       #   .carve/, .env, .dlt/secrets.toml, etc.
    dbt_project.yml.j2                                  #   only rendered when --with-dbt
    dbt_models/                                         #   greenfield dbt scaffold tree
        staging/.gitkeep
        marts/.gitkeep
        intermediate/.gitkeep
        sources.yml.j2
    dlt_sample/                                         #   only rendered when --with-dlt (a "hello world" dlt source)
        __init__.py.j2
        requirements.txt.j2
src/carve/integrations/dbt/convention_inference.py      # NEW — parse dbt_project.yml + walk models/ → markdown summary
src/carve/integrations/dlt/convention_inference.py      # NEW — scan el/ + .dlt/ → markdown summary
src/carve/integrations/inference/writer.py              # NEW — combines per-backend inferences into carve/conventions.md
src/carve/core/auth/bootstrap.py                        # NEW — generates initial API token; writes to .carve/token + state store
docs/getting-started-greenfield.md                      # NEW — walks a user through greenfield init → first plan
docs/getting-started-brownfield-dbt.md                  # NEW — walks a user with an existing dbt project through init
docs/getting-started-orchestration-only.md              # NEW — for users using Carve as orchestrator without authoring (PRD §6.2 mode 2)
tests/unit/test_init_detect.py                          # NEW
tests/unit/test_init_scaffold.py                        # NEW
tests/unit/test_convention_inference_dbt.py             # NEW
tests/unit/test_convention_inference_dlt.py             # NEW
tests/integration/test_init_greenfield.py               # NEW — fresh tempdir + carve init → expected layout
tests/integration/test_init_brownfield_dbt.py           # NEW — synthetic existing dbt project + init → integrated layout
tests/integration/test_init_brownfield_dlt.py           # NEW — synthetic existing dlt code + init → integrated layout
tests/integration/test_init_brownfield_mixed.py         # NEW — dbt + dlt brownfield together
tests/integration/test_init_separate_repo.py            # NEW — --dbt-url against a fixture git URL
tests/integration/test_init_idempotency.py              # NEW — re-running init doesn't clobber user edits
tests/integration/test_init_non_interactive.py          # NEW — --non-interactive with all flags supplied works in CI
```
