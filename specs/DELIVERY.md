...
**Increment 1 is complete** — layout, harness, extensibility, model-auth (as Increment 1b), and packaging are all shipped. **Increment 2 is complete** — the `init`, `memory`, and `sql` lean cores have all landed, with their heavier parts deferred to tracked issues. **Increment 3 is in progress** — the tool binder (the foundational unblock for declarative engineers), the dlt-engineer (substrate → callable skills → the agent + verification runner → the reviewers + review fan-out; only M1 `extract_load` retirement remains), and the **dbt-engineer's first slice (the `dbt_manifest` reader + the `parse_dbt_run` verify bridge)** have landed; the rest of the dbt-engineer (the agent, brownfield/greenfield, the `dbt-qa` reviewer) and the other Increment-3 capabilities are building toward their full specs. `dbt-execution` (net-new, unbuilt) + the live orchestrator wiring remain the cross-cutting blockers for both engineers' live verify-by-execution. The full corpus is internally consistent under the control-plane + AI-harness model.
