...
**Increment 1 is complete** — layout, harness, extensibility, model-auth (as Increment 1b), and packaging are all shipped. **Increment 2 is in progress**: the `init` and `memory` lean cores have landed (above); `sql` remains. Everything beyond is **designed but unbuilt**. The full corpus is internally consistent under the control-plane + AI-harness model.
