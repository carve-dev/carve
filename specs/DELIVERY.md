...
**Increment 1 is complete** — layout, harness, extensibility, model-auth (as Increment 1b), and packaging are all shipped. **Increment 2 is complete** — the `init`, `memory`, and `sql` lean cores have all landed (above), with their heavier parts deferred to tracked issues. Everything beyond (Increment 3 onward) is **designed but unbuilt**. The full corpus is internally consistent under the control-plane + AI-harness model.
