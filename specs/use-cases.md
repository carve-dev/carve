...
5. **Schedule auto-paused immediately.** The `schedules` row for salesforce is set `paused = true, paused_by = recovery, pause_reason = "retries exhausted: DataValidationError (data_type)"`. `schedule.paused` event fires (`source = recovery`); Slack: "⏸️ salesforce paused — investigating."
